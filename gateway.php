...
if (isset($_POST['salvar_pagviva'])) {
    $url = "https://pagviva.com/api/transaction/deposit";
    $ci = $_POST['pagviva_ci'];
    $cs = $_POST['pagviva_cs'];
    $ak = $_POST['pagviva_ak'];

    $id = 1;
    $stmt = $pdo->prepare("UPDATE pagviva SET url = ?, ci = ?, cs = ?, ak = ? WHERE id = ?");
    if ($stmt->execute([$url, $ci, $cs, $ak, $id])) {
        $_SESSION['success'] = 'Credenciais PagViva alteradas!';
    } else {
        $_SESSION['failure'] = 'Erro ao alterar as credenciais PagViva!';
    }
    header('Location: '.$_SERVER['PHP_SELF']);
    exit;
}
...
// Buscar dados do PagViva
$stmt = $pdo->query("SELECT url, ci, cs, ak FROM pagviva");
$pagviva = $stmt->fetch(PDO::FETCH_ASSOC);
?>
                    ...
                    <form method="POST">
                        ...
                        <div class="security-warning">
                            ...
                            <div class="security-warning-content">
                                <strong>Segurança:</strong> Suas credenciais são totalmente criptografadas e protegidas. Nunca compartilhe essas informações com terceiros. Para que os depósitos funcionem corretamente, crie um cadastro no gateway de pagamento acessando: https://pagviva.com/ Sistema feito por https://www.youtube.com/@Guizera6777
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="pagviva_ci" value="<?= $pagviva['ci'] ?? '' ?>" class="form-input" placeholder="Digite seu Client ID" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" name="pagviva_cs" value="<?= $pagviva['cs'] ?? '' ?>" class="form-input with-toggle" placeholder="Digite seu Client Secret" required id="clientSecret">
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" name="pagviva_ak" value="<?= $pagviva['ak'] ?? '' ?>" class="form-input with-toggle" placeholder="Digite sua API Key" required id="apiKey">
                            <button type="button" class="password-toggle" onclick="togglePassword('apiKey', 'toggleIcon')">
                                <i class="fas fa-eye" id="toggleIcon"></i>
                            </button>
                        </div>

                        <button type="submit" name="salvar_pagviva" class="submit-button">
                            <i class="fas fa-shield-alt"></i>
                            Salvar Credenciais
                        </button>
                    </form>
